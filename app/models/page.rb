class Page < ActiveRecord::Base
  ...
  def after_publish
    ...
    self.connectors.for_page_version(self.version).find(:all,:order => "position").each do |c|
  ...
  def copy_connectors(options={})
    connectors.for_page_version(options[:from_version_number]).find(:all,:order => "connectors.container, connectors.position").each do |c|
