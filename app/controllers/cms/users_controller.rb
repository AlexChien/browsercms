class Cms::UsersController < Cms::ResourceController
  ...
  def index
    ...
    unless params[:key_word].blank?
      query << %w(login email name).collect { |f| "lower(#{f}) LIKE lower(?)" }.join(" OR ")
      3.times { conditions << "%#{params[:key_word]}%" }
    end
    ...
    @users = User.cms_paginate(:page => params[:page], :per_page => per_page, :include => :user_group_memberships, :conditions => conditions, :order => "name, email")
  end
